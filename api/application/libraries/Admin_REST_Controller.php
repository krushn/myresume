 ...
 * @version         3.0.0-pre
 */
abstract class Admin_REST_Controller extends CI_Controller
{
    /**
    ...
    */
    ...
    public function __construct($config = 'rest')
    {
        ...
        //visitor log 
        $logged = $this->rest->db->count_all_results('visitor_log', array('ip_address'=> $_SERVER['REMOTE_ADDR']));
        
        if ($logged) {
            $this->rest->db->update('visitor_log', array('date' => date('Y-m-d')), array('ip_address'=> $_SERVER['REMOTE_ADDR']));
        } else {
            $this->rest->db->insert('visitor_log', array('ip_address'=> $_SERVER['REMOTE_ADDR'], 'date' => date('Y-m-d')));
        }   
    ...
    protected function _check_login($username = '', $password = FALSE)
    {
        ...
        $auth_source = strtolower('is_admin_login');
        $rest_auth = strtolower('session');
        $valid_logins = $this->config->item('rest_valid_logins');

        if (!'is_admin_login' && $rest_auth == 'digest') { // for digest we do not have a password passed as argument
